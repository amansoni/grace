...
import { SET_AUTH, CHANGE_FORM, SENDING_REQUEST, SET_ERROR_MESSAGE, USER_PROFILE } from '../constants/AppConstants';
 ...
 * @param  {string} username The username of the new user
 * @param  {string} password The password of the new user
 * @param  {string} email The password of the new user
 * @param  {string} firstName The password of the new user
 * @param  {string} surname The password of the new user
 */
export function setProfile(username, password, email, firstName, surname) {
  return (dispatch) => {
    // Show the loading indicator, hide the last error
    dispatch(sendingRequest(true));
    // If no username or password was specified, throw a field-missing error
    if (anyElementsEmpty({ username, password, email, firstName, surname })) {
      dispatch(setErrorMessage(errorMessages.FIELD_MISSING));
      dispatch(sendingRequest(false));
      return;
    }
